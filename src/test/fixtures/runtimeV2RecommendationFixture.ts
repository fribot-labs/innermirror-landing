...
export const RUNTIME_V2_RECOMMENDATION_FIXTURE_INSIGHT_GENERATED_AT =
...
/**
 * Creates a Runtime V2 Recommendation Insight fixture.
 *
 * Recommendation Insight is generated by the private Runtime.
 * This fixture only reproduces the transport result for Landing
 * presentation and integration tests.
 */
export function createRuntimeV2RecommendationInsightFixture(
  overrides:
    Partial<RuntimeV2RecommendationInsight> = {},
  ...
  const insight = {
    ...
    headline:
      "Recommendation direction is stable, but evidence remains limited.",

    summary:
      "Runtime has repeated the same Recommendation direction while recent project activity has not yet provided strong confirmation.",

    keyInsight:
      "Recommendation confidence is currently limited more by missing aligned evidence than by instability in the Recommendation direction.",

    supportingReasons: [
      "Comparison: Direction maintained",
      "Observation: Recommended direction is not yet visible",
    ...
    evidence: [
      {
        id:
          "recommendation-insight-evidence-ri13",

        source:
          "recommendation-observation",

        label:
          "Recommendation Insight Evidence",

        summary:
          "The current Recommendation direction is stable, but additional aligned project evidence is still required.",
    ...
    generatedAt:
      RUNTIME_V2_RECOMMENDATION_FIXTURE_INSIGHT_GENERATED_AT,

    ...overrides,
  } satisfies RuntimeV2RecommendationInsight;
...
export type CreateRuntimeV2RecommendationResultFixtureOptions = {
  ...
  /**
   * Insight is excluded by default so that existing fixtures remain
   * compatible with Runtime responses created before PR-RI13.
   */
  includeInsight?:
    boolean;
 ...
 * - no Insight
 *
 * Prediction and Insight are enabled explicitly so that older tests
 * remain unchanged.
 */
...
export function createRuntimeV2RecommendationTimelineResultFixture():
  RuntimeV2RecommendationIntegrationResult {
